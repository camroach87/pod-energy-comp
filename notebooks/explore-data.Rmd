...
demand_df <- read_csv("../data/pod_ds_task0/demand_train_set0.csv")
weather_df <- read_csv("../data/pod_ds_task0/weather_train_set0.csv")
pv_df <- read_csv("../data/pod_ds_task0/pv_train_set0.csv") %>% 
...
Many papers already explore this.
...
* Need to forecast within limits (0-3.8?) for solar PV. Apparently it's a 5 MW solar farm but only 3.8 MW are produced at most due to system losses. Use historical max PV generation in order to determine appropriate upper bound.
  * I suspect using an inverse logit link function and logistic regression may be the best way to proceed with this data.
* Try interaction variable between irradiance and panel temperature.
* If using GBM don't need to worry about forcing limits since tree based method will do this automatically (doesn't extrapolate).



## GBM

* Unsurprisingly, GBM seems to work well for prediction. Should use it's out of bag error metrics for hyperparameter tuning to make fitting efficient.


# TODO

* Check if period of day improves model fit.
...
* For PV prediction - include differences in temp and irradiance between sites? Maybe just use two closest sites for this.
* Forecast within limits - transform response so that GBM only can predict within 0 and 3.8 (or max historical pv generation).